...
labels = [
    ...
    "$cyber threat",
    "$cybercrime and cyberwarfare likelihood",
    "$data security",
    "$email security",
    "$encryption and cryptography security",
...
client = pymongo.MongoClient(
    "mongodb+srv://SeChatUsername:SeChatLover%23123@cbs-01.1swurq8.mongodb.net/"
)
...
class MyServer(CORSRequestHandler):
    def do_POST(self):
        if self.path == "/encrypt_decrypt":
            ...
            out = ""
            if (
                parsed_data["shared_key"] != None
                and parsed_data["shared_key"].strip() != ""
                and parsed_data["mode"] != None
                and parsed_data["mode"].strip() != ""
                and parsed_data["message"] != None
                and parsed_data["message"].strip() != ""
            ):
            ...
            if (
                ...
                and parsed_data["user_id"].strip() != ""
                and parsed_data["shared_user_id"] != None
                and parsed_data["shared_user_id"].strip() != ""
            ):
                user_id = parsed_data["user_id"].encode()
                shared_user_id = parsed_data["shared_user_id"].encode()
                userA_private = UserData.find({"user_id": {"$eq": int(user_id)}})[0][
                    "private_key"
                ]
                userB_public = UserData.find({"user_id": {"$eq": int(shared_user_id)}})[
                    0
                ]["public_key"]
                a = DiffieHellman()
            ...
            if parsed_data["message"] != None and parsed_data["message"].strip() != "":
                ...
                response = {"message": message, "label": labels[np.argmax(pred)]}
                self.wfile.write(json.dumps(response).encode())
            else:
                self.send_response(400)
                self.send_header("Content-type", "application/json")
            ...
            if (
                ...
                and parsed_data["username"].strip() != ""
                and parsed_data["password"] != None
                and parsed_data["password"].strip() != ""
            ):
                ...
                user_login_query = {
                    ...
                    "user_password": {"$eq": hash_password},
                }
                userQuery = UserData.find(user_login_query)
                query_data = []
                query_data = [parse_json(x) for x in userQuery]
                if len(query_data) <= 0:
                    response = {"data": query_data, "status": 400}
                else:
                    response = {"data": query_data, "status": 200}
                self.wfile.write(json.dumps(response).encode())
            else:
                self.send_response(400)
                self.send_header("Content-type", "application/json")
        ...
        elif self.path == "/getConversation":
            content_length = int(self.headers["Content-Length"])
            post_data = self.rfile.read(content_length).decode("utf-8")
            parsed_data = json.loads(post_data)
            if (
                parsed_data["conversation_id"] != None
                and parsed_data["conversation_id"].strip() != ""
            ):
                ...
                conversationQuery = ConversationData.find(conversation_query)
                query_data = []
                query_data = [parse_json(x) for x in conversationQuery]
                if len(query_data) <= 0:
                    response = {"data": query_data, "status": 400}
                else:
                    response = {"data": query_data, "status": 200}
            ...
            if (
                ...
                user_data_uery = UserData.find(user_query)
                query_data = []
                query_data = [parse_json(x) for x in user_data_uery]
                if len(query_data) <= 0:
                    response = {"data": query_data, "status": 400}
                else:
                    response = {"data": query_data, "status": 200}
